...
const Button = ({
  ...
  const loadingState = loading ? 'opacity-80 cursor-not-allowed' : '';
  const disabledStyle = disabled ? 'opacity-60 cursor-not-allowed' : '';

  return (
    <button
      className={`${baseStyles} ${variantStyles[variant]} ${sizeStyles[size]} ${widthStyle} ${loadingState} ${disabledStyle} ${className}`}
