  ...
  const AddCourseForm: React.FC<AddCourseFormProps> = ({ onClose }) => {
    ...
    // Reset file error before setting new file
    setErrors(prev => ({ ...prev, file: '' }));

    // Handle setting the file
    setFile(file);
  };
    ...
    try {
      let uploadedFileNameOrUrl = '';

      if (file) {
        const formData = new FormData();
        formData.append('file', file);

        const uploadResponse = await fetch('https://localhost:7130/api/FileUpload/course-material', {
          method: 'POST',
          body: formData,
        });
        ...
        uploadedFileNameOrUrl = uploadData.fileUrl || uploadData.fileName; // adjust to match API response
      }
      ...
      const coursePayload = {
        ...
        teacher_id: '3fa85f64-5717-4562-b3fc-2c963f66afa6', // replace as needed
        thumbnail: uploadedFileNameOrUrl || 'default-thumbnail.png',
        category: formData.category,
        duration: parseInt(formData.duration),
        level: formData.level,
      };

      const courseResponse = await fetch('https://localhost:7130/api/Courses', {
        method: 'POST',
        headers: {
